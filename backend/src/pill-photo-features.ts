import { z } from "zod";
import { pillObservationSchema, searchPillCandidates, type PillCatalog } from "./pill-identification.ts";

export const PILL_PHOTO_PROMPT_VERSION = "pill-photo-observation-v1";
export const pillPhotoFeaturesSchema = z.object({
  observation: pillObservationSchema.omit({ source: true }).extend({
...
Return the specified JSON only. Missing or ambiguous information must stay unknown, null, or uncertain; never invent a missing side.
...
Copy imprint letters/digits/symbols faithfully, preserving 0 versus O, 1 versus I, punctuation and script. A null imprint means unreadable or unseen. An empty string means that the visible surface was inspected and has no letters/digits. Do not treat a logo as a readable letter.
If letters are partly visible but cannot be reliably transcribed, use null, not an invented completion.
