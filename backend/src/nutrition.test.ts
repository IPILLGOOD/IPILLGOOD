...
test("확정한 고혈압은 질환별 식사/영양 주제를 만든다", () => {
  ...
  assert.equal(
    insights.some(
      (insight) =>
        insight.id === "hypertension-lower-sodium" && insight.kind === "food",
    ),
    true,
  );
  assert.equal(
    insights.some(
      (insight) =>
        insight.id === "hypertension-supplements-not-treatment" &&
        insight.kind === "food",
    ),
    true,
  );
  assert.equal(
    insights.every(
      (insight) =>
        insight.evidence.length > 0 && insight.lastReviewedAt === "2026-08-29",
    ),
    true,
  );
});
...
test("주의 약물이 있으면 칼륨 항목을 더 보수적인 피하기 상태로 올린다", () => {
  ...
  const insight = buildNutritionInsights(input).find(
    (item) => item.id === "hypertension-supplements-not-treatment",
  );
