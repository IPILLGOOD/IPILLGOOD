...
export async function syncMedicationReminderSchedules(input: {
  ...
  return firestore.runTransaction((tx) => syncMedicationReminderSchedulesInTransaction({ ...input, firestore, now, tx }));
}

/** Subscription activation and its schedules commit together, with all reads before writes. */
export async function syncMedicationReminderSchedulesInTransaction(input: {
  recipientId: string; firestore: FirestoreLike; now: Date; tx: TransactionLike; hasActiveSubscription?: boolean;
}) {
  const { firestore, now, tx } = input;
  ...
  const active = input.hasActiveSubscription ?? subscriptions.docs.some((doc) => (doc.data() as { active?: boolean }).active);
