// Worker-compatible request construction, transport and feature extraction. No native image decoder.
...
import type {
  PillPhotoOcrRotationViews, PillPhotoPreprocessingVariants, PillPhonePhotoPreprocessingVariants,
...
const inputImage = (bytes: Buffer) => ({
  ...
  image_url: `data:image/${bytes[0] === 0xff && bytes[1] === 0xd8 ? "jpeg" : "png"};base64,${bytes.toString("base64")}`,
