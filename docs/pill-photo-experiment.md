...
- 후보 생성은 각인 근거를 우선한다. 제형·모양·색상·분할선은 알려진 불일치를 `conflicts`에 남겨 `strong`/`possible` 등급과 순서를 결정하지만, 각인이 맞는 후보를 색상 오분류 하나만으로 삭제하지 않는다.
...
최초 사진 연결 단계에는 기본 14개 + 로컬 공개 자료 테스트 4개를 추가했다. 팀 공통 자료 단계에서 고정 카탈로그·기록 결과·오프라인 재생·만료 경계 검증 5개를 더했다. 리뷰 반영 단계에서는 복수 각인 계약·각인 우선 재정렬·마스크 품질·회귀 diff 테스트를 보강했다. 최종 전체 단위 테스트는 **프론트 89 + 백엔드 284 = 373개**, 실패·건너뜀 0개다. `pill:regression` 필수 게이트 6/6, 프로젝트 타입 검사, 변경 백엔드 파일 strict 타입 검사, ESLint, `git diff --check`도 통과했다. 모든 사진 재생·회귀 검사는 외부 `fetch`를 차단하거나 요청 0회를 확인한다.
