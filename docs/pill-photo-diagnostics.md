...
- 판독/결합 문제, 안전 게이트, 정답 후보/보류/상위20 밖/검색 규칙상 탈락을 분리한다. ‘정답 품목만 넣는 검색’은 탈락 여부를 확인하는 **라벨 참조 진단**에 한정하며, recall과 사용자 후보는 항상 전체 카탈로그 검색으로 계산한다.
...
1. 실험 조건·실행 버전을 고정하고 실제 전송 이미지 및 단계별 관찰을 재현할 기록 구조를 보완한다. 동일 조건 반복 측정과 전처리 변경 비교를 구분한다.
