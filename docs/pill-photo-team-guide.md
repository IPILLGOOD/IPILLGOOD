...
의존성 설치 후 위 검증·재생은 별도 다운로드/ZIP/API 키 없이 작동한다. `.env.local`을 복사해 공유하지 않는다. 전체 `npm test`에도 공유 자료 검증이 포함된다.

콘솔의 `directory`에 생성된 `report.html`을 브라우저에서 직접 연다. 사진 A/B, 저장된 추출 특징, 공식 정답 참고 특징, 후보/보류와 이유가 보인다. HTML과 이미지가 함께 생성되므로 별도로 사진을 복사할 필요가 없다. 웹 앱의 촬영 화면을 추가한 것은 아니다.
...
| 항목 | 최초 실험 |
| --- | ---: |
| 정상 4건의 기대 품목 후보 포함 | 0/4 |
| 예외 사진 재촬영 반환 | 2/2 |
| 기대한 예외 원인까지 감지 | 1/2 |

따라서 기능 완성이 아니라 성능 개선을 시작할 기준선이다. `pill:verify` 통과는 무결성·동작 계약을 검증한 것이지 약을 정확히 식별했다는 의미가 아니다. 검색 코드를 개선하면 새 재생 결과는 바뀔 수 있으며, 테스트는 과거 0/4를 강제하지 않고 저장 특징과 현재 검색기의 연결을 검증한다. [실험 기록](pill-photo-experiment.md)에 개별 실패 원인이 있다. 고정 사진에만 맞춰 수정한 뒤 그 수치를 독립 평가 정확도로 보고하지 않는다.
...
| 후보 검색·예외·평가 | `backend/src/pill-identification.ts`, `pill-form-policy.ts`, `backend/scripts/pill-photo.ts` | 같은 입력의 후보/보류 근거와 잘못된 후보 제시 방지 회귀 검사 |
...
5. PR에는 변경 범위·기준 대비 결과·남은 한계·재현 명령을 적는다. #123의 사용자 기능은 아직 미완료이므로 완료/출시로 표현하지 않는다.
