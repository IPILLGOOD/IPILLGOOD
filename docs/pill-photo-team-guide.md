> 배포 웹 연결: `/medications/photo` 업로드 화면과 Workers API를 추가했습니다. 기존 Node/Sharp 진입점과 구분되는 브라우저 전처리를 사용하며, 최신 공식 목록과 승인된 API 키가 필요합니다. [배포·검증 안내](pill-photo-web.md)를 참고하세요.
...
**현재 공통 함수는 Node.js 24와 Sharp가 필요한 실행 경로다.** Sharp 0.35.4를 런타임 의존성으로 선언했다. 통합 검증에서 보안 패치를 적용했으며, 기존 전처리 회귀 테스트도 통과했다. `@care-atlas/backend` 기본 진입점에서 재수출하지 않아 기존 서버 코드가 이 경로를 자동으로 가져오지 않는다. 이 변경만으로 Cloudflare Workers에서 Sharp 전처리가 실행되지는 않는다. 실행 환경 또는 전처리 연결 방식을 배포 담당자가 결정해야 하며, 이번 변경은 HTTP API 경로·화면·배포 설정을 추가하지 않는다.
