...
`/medications/photo`에서 같은 알약의 앞뒤 사진을 선택한다. 모바일 하단의 **빠른 기록 → 사진으로 약 검색**, 또는 복용약 화면의 **알약 사진으로 비교 후보 찾기** 링크로 진입한다. 하단 메뉴에는 오늘 할 일·복용약·빠른 기록·식사/영양을 표시하며, 대시보드·안부 확인·문서는 빠른 기록에서 연다. 분석 결과에서 자동으로 복용약을 등록하지 않는다.

## 처리 흐름

1. 브라우저가 JPEG·PNG를 디코딩하고 EXIF 방향을 반영한다. 원본은 한 장당 5MiB·2,500만 화소 이하만 받는다. 알약 전체가 사진 중앙 40% 정사각형 안에 들어오도록 안내한다.
2. 기기에서 문맥 이미지(긴 변 최대 1024px), 중앙 확대(768px), 명암 대비 이미지, 각 이미지의 0/90/180/270도 회전본을 JPEG로 다시 만든다. Canvas 인코더가 다시 붙인 EXIF(WebKit 포함)·XMP·IPTC·주석도 전송 전에 제거하고, ICC 색상 프로필과 이미지 픽셀은 유지한다. 원본 EXIF·위치 정보·파일명은 전송하지 않는다. 전처리 버전은 `pill-web-canvas-jpeg-v1`이다. Node/Sharp 전처리와 픽셀 단위로 같다고 주장하지 않는다.
