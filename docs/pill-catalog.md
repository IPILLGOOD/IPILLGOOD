...
- `--max-age-hours`는 반드시 명시한다(정수 1~720). 예제의 24는 로컬 검증 시 사용할 경과 시간 한도이며, 의료적 안전성이나 운영 갱신 주기의 확정값이 아니다.
...
배포·검색 모두 30일 이내의 검증 자료를 요구한다. 자동 갱신은 없어 30일 이내에 목록을 갱신해 재배포해야 한다. 오래되거나 손상된 자료를 과거 테스트 카탈로그나 일부 표본으로 대체하지 않는다. 고정 평가 카탈로그는 원래 날짜를 보존하고 테스트 전용 시계로만 재생한다.
