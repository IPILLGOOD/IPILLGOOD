...
import {
  checkChoice,
  dismissInstallPromptWhenShown,
} from "../test-support/browser-controls";
...
async function syntheticGoogleToken(userId: string) {
  ...
  return new SignJWT({ name: "자동 검증 계정", provider: "google" })
...
test.beforeEach(async ({ context, page }) => {
  ...
  page.on("console", (message) => {
    if (message.type() === "error")
      events.push(`console: ${message.text().slice(0, 500)}`);
  });
  page.on("pageerror", (error) =>
    events.push(`pageerror: ${error.message.slice(0, 500)}`),
  );
  page.on("requestfailed", (request) =>
    events.push(
      `requestfailed: ${new URL(request.url()).pathname} ${request.failure()?.errorText}`,
    ),
  );
...
test.afterEach(async ({ page }, testInfo) => {
  await testInfo.attach("browser-events", {
    body: JSON.stringify(browserEvents.get(page) ?? [], null, 2),
    contentType: "application/json",
  });
  expect(
    (browserEvents.get(page) ?? []).filter((event) =>
      event.startsWith("pageerror:"),
    ),
  ).toEqual([]);
});

test("demo: check-in, document create/delete, reload, dashboard/report and logout cleanup", async ({
  page,
  context,
}) => {
  ...
  try {
    ...
    await page
      .getByRole("link", { name: "오늘 화면으로 돌아가기", exact: true })
      .click();
    await expect(page).toHaveURL(/\/today$/);
    await page.goto("/documents");
    const before = await page.locator(".document-item").count();
    const recipient = fixture.admin
      .collection("careRecipients")
      .doc(recipientId!);
    const baselineMedicationCount = (
      await recipient.collection("medicationPlans").get()
    ).size;
    const baselineDoseEventCount = (
    ...
    for (const documentType of ["처방전", "진단서"]) {
      ...
      if (documentType === "처방전") {
        ...
        await diagnosisReview
          .getByRole("button", { name: "수정값 저장" })
          .click();
        await expect(
          diagnosisReview.getByText("진단 정보를 수정했어요.", {
            exact: false,
          }),
        ).toBeVisible();
        await expect(page.locator(".diagnosis-confirmation")).toContainText(
          "본태성 고혈압",
        );
      }
      await expect
        .poll(() =>
          page.evaluate(() =>
            sessionStorage.getItem("ipillgood:document-analysis-job"),
          ),
        )
        .toBeNull();
      ...
      if (documentType === "처방전") {
        expect((await recipient.collection("medicationPlans").get()).size).toBe(
          baselineMedicationCount,
        );
        expect((await recipient.collection("doseEvents").get()).size).toBe(
    ...
    for (let index = 0; index < (await medicationTabs.count()); index++) {
      ...
      await expect(panel.getByRole("heading", { level: 3 })).toHaveText(
        await tab.locator("strong").innerText(),
      );
      await expect(
        panel.getByRole("link", { name: /상세 정보 보기/ }),
      ).toBeVisible();
    }
    const subscriptions = await fixture.admin
      .collection("pushSubscriptions")
      .where("recipientId", "==", recipientId)
      .get();
    const schedules = await fixture.admin
      .collection("medicationReminderSchedules")
      .where("recipientId", "==", recipientId)
      .get();
    ...
    if (recipientId) {
      await fixture.admin.recursiveDelete(
        fixture.admin.collection("careRecipients").doc(recipientId),
      );
      await fixture.admin
        .collection("careReadModels")
        .doc(recipientId)
...
test("synthetic account: isolated normal session, read-only Push status, forged auth rejected", async ({
  context,
  page,
  request,
}) => {
  ...
  try {
    expect(
      (
        await request.get("/api/push/subscriptions?deviceId=test-device-000001")
      ).status(),
    ).toBe(401);
    ...
    await context.addCookies([
      {
        name: "care_atlas_session",
        value: token,
        url: process.env.IPILLGOOD_TEST_BASE_URL!,
        httpOnly: true,
        sameSite: "Lax",
      },
    ]);
    ...
    const initialRecipient = await fixture.admin
      .collection("careRecipients")
      .doc(recipientId)
      .get();
    expect(initialRecipient.data()).toMatchObject({
      displayName: "",
      ageBand: "",
      consentConfirmed: false,
    });
    const beforeProfileAnalysis = await context.request.post(
    ...
    expect((await beforeProfileAnalysis.json()).message).toContain(
      "돌봄 대상자 정보",
    );
    ...
    await expect(page).toHaveURL(/\/today$/);
    await expect(
      page.getByRole("heading", { name: "돌봄 기록을 시작해 볼까요" }),
    ).toBeVisible();
    await expect(
      page.getByRole("link", { name: "첫 문서 등록하기" }),
    ).toHaveAttribute("href", "/documents");
    await expect(
      page.getByRole("form", { name: "오늘의 안부 바로 기록" }),
    ).toHaveCount(0);
    ...
    const { sessionKey } = await (
      await context.request.get("/api/push/config")
    ).json();
    const status = await context.request.get(
      "/api/push/subscriptions?deviceId=test-device-000001",
      { headers: { "x-push-session": sessionKey } },
    );
    ...
    expect(model.data()?.recipient).toMatchObject({
      displayName: "온보딩 검증 대상자",
      ageBand: "75",
      consentConfirmed: true,
    });
    expect(model.data()?.recipient.profileCompletedAt).toEqual(
      expect.any(String),
    );
    ...
    if (emulatorToken)
      await request.post(
        `http://${process.env.FIREBASE_AUTH_EMULATOR_HOST}/identitytoolkit.googleapis.com/v1/accounts:delete?key=synthetic`,
        { data: { idToken: emulatorToken } },
      );
    await fixture.admin.recursiveDelete(
      fixture.admin.collection("careRecipients").doc(recipientId),
...
test("documents: samples stay demo-only across API requests, uploads and account switches", async ({
  context,
  page,
  request,
}, info) => {
  ...
  try {
    expect(
      (
        await request.post("/api/documents/analyze", {
          multipart: { documentType: "처방전", sample: "true" },
        })
      ).status(),
    ).toBe(401);
    ...
    for (const width of [320, 768, 1024, 1440]) {
      ...
      await expect(page.locator(".sample-button, .sample-divider")).toHaveCount(
        0,
      );
      await expect(
      ...
      if (width === 320 || width === 1440) {
        await info.attach(`normal-account-documents-${width}`, {
    ...
    for (const documentType of ["처방전", "진단서"]) {
      ...
      expect((await forbidden.json()).message).toBe(
        "샘플 문서 체험은 데모 로그인에서만 이용할 수 있어요.",
      );
    }
    const sampleWithFile = await context.request.post(
      "/api/documents/analyze",
      {
    ...
    expect(
      (
        await context.request.post("/api/documents/analyze", {
          multipart: { documentType: "처방전" },
        })
      ).status(),
    ).toBe(400);
    ...
    const analysisResponse = page.waitForResponse(
      (response) =>
        new URL(response.url()).pathname === "/api/documents/analyze" &&
    ...
    for (const id of [recipientId, demoRecipientId].filter((id): id is string =>
      ...
      await fixture.admin.recursiveDelete(
        fixture.admin.collection("careRecipients").doc(id),
      );
