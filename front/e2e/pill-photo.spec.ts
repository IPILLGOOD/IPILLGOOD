...
test("pill photo: actual canvas preprocessing, consent, bounded upload, result, errors and mobile reflow", async ({ page }) => {
  let requests = 0;
  let mode: "result" | "error" | "retake" = "result";
  await page.route("**/api/pills/analyze", async route => {
    if (route.request().method() === "GET") return route.fulfill({ json: { ready: true } });
    requests++;
    // Validate the bytes produced by the real browser with the actual server upload parser.
    ...
    if (mode === "error") return route.fulfill({ status: 503, json: { message: "사진 분석을 완료하지 못했어요. 잠시 후 다시 시도해주세요." } });
    return route.fulfill({ json: { status: "completed", preprocessingVersion: PILL_WEB_PREPROCESSING_VERSION,
  ...
  await page.getByRole("link", { name: "알약 사진으로 비교 후보 찾기" }).click();
  ...
  await page.screenshot({ path: "verification-artifacts/pill-photo-390.png", fullPage: true });
  ...
  await page.evaluate(() => scrollTo(0, 0));
  await page.screenshot({ path: "verification-artifacts/pill-photo-320-large-text.png", fullPage: true });
...
test("pill photo: unavailable catalog blocks upload, and real API enforces origin/session", async ({ page, request }) => {
  ...
  await page.screenshot({ path: "verification-artifacts/pill-photo-unavailable.png", fullPage: true });
});
