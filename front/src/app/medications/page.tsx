...
export default async function MedicationsPage({ searchParams }: { searchParams: Promise<{ q?: string | string[]; add?: string | string[] }> }) {
  ...
  const [snapshot, result] = await Promise.all([
    ...
    query ? rateLimit?.allowed ? withCareAccountProcessing(scope.recipientId, () => searchOfficialMedicationInfo(query, { openAiApiKey: "" })) : Promise.resolve(limitedResult) : Promise.resolve(null),
  ]);
      ...
      {medications.length > 0 ? (
        <MedicationCabinet
          ...
          medications={medications.map((medication) => ({
            ...
            needsExplanation: medication.purposePlain === "처방 목적은 의사나 약사에게 확인해주세요." && medication.descriptionPlain === "식약처 검색 결과에서 사용자가 직접 등록한 복용약이에요.",
