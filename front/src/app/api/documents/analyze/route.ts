...
const allowedDocumentTypes = new Set<ClinicalDocumentType>(["처방전 또는 약봉투", "처방전", "약봉투", "진단서"]);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!allowedDocumentTypes.has(documentType as ClinicalDocumentType)) {
      return Response.json({ message: "처방전·약봉투 또는 진단서를 선택해주세요." }, { status: 400 });
    }

    const file = formData.get("document");
    if (!isSample && (!(file instanceof File) || file.size === 0)) {
      return Response.json(
        { message: "분석할 처방전·약봉투 또는 진단서 파일을 선택해주세요." },
        { status: 400 },
      );
    }
    ...
    const contentHasher = createHash("sha256").update(typedDocumentType).update("\0");
    if (diagnosisName) contentHasher.update(diagnosisName).update("\0");
    const contentHash = contentHasher.update(fileBytes ?? `sample:${fileName}`).digest("hex");
