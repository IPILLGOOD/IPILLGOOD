...
export function recentCareRecords(records: Records, now = new Date()) {
  ...
  return {
    ...
    symptomEvents: records.symptomEvents.filter((item) =>
...
export function adherenceSummary(events: DoseEvent[]) {
  ...
  const confirmed = answerable.filter(
    (event) => event.response === "completed",
  ).length;
  const rate =
    answerable.length === 0
      ? null
      : Math.round((confirmed / answerable.length) * 100);
...
export function observationEvidenceLabel(
  evidence: ObservationEvidence | undefined,
  role?: "caregiver" | "recipient",
) {
  const value =
    evidence ?? (role === "recipient" ? "self_reported" : "caregiver_observed");
  return {
    self_reported: "이용자 자가보고",
...
export function observationEvidenceCounts(
  events: Array<DoseEvent | SymptomEvent>,
) {
  ...
  for (const event of events) {
    const role =
      "answeredBy" in event
        ? event.answeredBy
        : (event.actorRole ??
          (event.reporterType === "recipient_reported"
            ? "recipient"
            : "caregiver"));
...
export function careTimelineItems(
  medications: MedicationPlan[],
