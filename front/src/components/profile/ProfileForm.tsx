...
export function ProfileForm({
  ...
  const [adoptedSave, setAdoptedSave] = useState<ActionState | null>(null);
  const conflictRefreshed = useRef(false);
  const identitySection = useRef<HTMLDetailsElement>(null);
  const healthSection = useRef<HTMLDetailsElement>(null);
  useEffect(() => {
  ...
  useEffect(() => {
    if (
      ...
      state.fieldErrors?.ageBand?.[0]
    ) {
      if (identitySection.current) identitySection.current.open = true;
    }
    if (state.fieldErrors?.confirmedConditions?.[0] && healthSection.current) {
      healthSection.current.open = true;
    }
  }, [state.fieldErrors]);
  ...
  return (
    <form
      action={action}
      className="profile-accordion-form"
    ...
    >
      ...
      {state.conflict ? (
        <button
          ...
          type="button"
          disabled={revision === baselineRevision}
          onClick={() => setBaselineRevision(revision)}
        >
          {revision === baselineRevision
            ? "최신 내용 불러오는 중…"
            : "최신 내용 확인 후 다시 저장"}
      ...
      <details ref={identitySection} className="profile-accordion" open={onboarding}>
        <summary>
          <span>
            <strong>기본 정보</strong>
            <small>이름, 나이, 신체 정보</small>
          </span>
          <ChevronDown size={20} aria-hidden="true" />
        </summary>
        <div className="form-grid profile-accordion__content">
          <div className="field">
            <label htmlFor="displayName">
              화면에 표시할 이름 <span aria-hidden="true">*</span>
            </label>
            <input
              id="displayName"
              name="displayName"
          ...
          <div className="field">
            <label htmlFor="ageBand">
              나이 <span aria-hidden="true">*</span>
            </label>
            <input
              id="ageBand"
              name="ageBand"
            ...
            {error("ageBand") ? (
              <p className="field-error" id="ageBand-error">
                {error("ageBand")}
              </p>
            ) : null}
          </div>
          ...
          <div className="field">
            ...
            <p className="field-hint" id="heightCm-hint">
              선택 입력 · 프로필 참고 정보로만 보관하며 복용량을 자동 계산하지
              않아요.
            </p>
          </div>
          ...
          <div className="field">
            ...
            <p className="field-hint" id="weightKg-hint">
              선택 입력 · kg 단위로 프로필에 보관하며 복용량을 자동 계산하지
              않아요.
            </p>
          </div>
        </div>
      ...
      <details ref={healthSection} className="profile-accordion">
        <summary>
          <span>
            <strong>건강 정보</strong>
            <small>알레르기, 참고 메모, 확정 질환</small>
          </span>
          <ChevronDown size={20} aria-hidden="true" />
        </summary>
        <div className="form-grid profile-accordion__content">
          <div className="field">
            ...
            <p className="field-hint" id="allergies-hint">
              보호자가 복약 전 주의할 정보를 함께 확인할 수 있도록 프로필에
              정리해요.
            </p>
          </div>
          ...
          <div className="field">
            ...
            <p className="field-hint" id="conditions-hint">
              참고용 자유 메모예요. 이 내용만으로 식사/영양 고려사항을 만들지는
              않아요.
            </p>
          </div>
          ...
          <ConfirmedConditionsEditor
            ...
            conditions={recipient.confirmedConditions ?? []}
            error={error("confirmedConditions")}
          />
        </div>
      </details>
      ...
      <details className="profile-accordion">
        <summary>
          <span>
            <strong>생활과 돌봄 메모</strong>
            <small>이동 상태와 보호자가 남길 내용</small>
          </span>
          <ChevronDown size={20} aria-hidden="true" />
        </summary>
        <div className="form-grid profile-accordion__content">
          ...
          <div className="field form-grid__wide">
            ...
            <p className="field-hint" id="caregiverNote-hint">
              식사·수면·복약 습관 등 생활 맥락을 보호자와 함께 확인할 수 있도록
              보관해요.
            </p>
          </div>
        </div>
      ...
      <div className="profile-consent-inline">
        <label className="consent-row">
          <input
            ...
            aria-describedby={
              error("consentConfirmed") ? "consent-error" : undefined
            }
          />
          <span>건강정보 저장에 동의합니다.</span>
        </label>
