...
export function DocumentAnalysisResult({
  ...
  const isMedicationDocument = analysis.documentType !== "진단서";
  ...
  const medicationsNeedingReview =
    analysis.medications?.filter(
    ...
    >
      <div className="analysis-result__header">
        ...
        <div>
          ...
          <h3 id="analysis-result-title">분석 결과</h3>
          {!isMedicationDocument ? <p>{analysis.summary}</p> : null}
        </div>
      </div>

      {!isMedicationDocument ? <dl className="analysis-findings">
      ...
      {analysis.extraction?.status !== undefined &&
      analysis.extraction.status !== "complete" ? (
        <div
          className="disease-lookup-status disease-lookup-status--failed"
          role="status"
        >
          ...
          <p>
            ...
            찾지 못한 값은 버리지 않고 검토 초안으로 남겼어요. 원본을 보며
            수정하거나 직접 추가해주세요.
          </p>
        </div>
      ) : null}

      {isMedicationDocument && analysis.medications?.length ? (
        <div
          ...
          <p>
            ...
            {medicationRegistration === "merged"
              ...
              : medicationRegistration === "pending"
                ? "등록 방식을 선택해주세요."
                : requiresMedicationVerification
                  ? `확인이 필요한 약 ${medicationsNeedingReview}개가 있어요.`
                : requiresPeriodReview
                  ? "복용 기간을 확인해주세요."
                    : "아래에서 확인해주세요."}
          </p>
      ...
      {isMedicationDocument && analysis.medications?.length ? (
        <ol className="analysis-medication-simple" aria-label="문서에서 찾은 약">
          {analysis.medications.map((medication, index) => (
            <li key={`${medication.productName}-${index}`}>
              <strong>약 {index + 1}</strong>
              <span>{medication.reviewStatus === "verified" ? "확인됨" : "확인 필요"}</span>
            </li>
          ))}
        </ol>
      ) : null}
      ...
      documentId &&
      nutritionDiagnoses.length > 0 ? (
        <form
          action={confirmDiagnosesAction}
          className="diagnosis-confirmation"
        >
          ...
          <div>
            ...
            <p>
              {nutritionDiagnoses
                ...
                .join(" · ")}
              을(를) 의료진에게 확인받은 질환으로 저장해요. 원본과 비교한 뒤
              확정해주세요.
            </p>
          </div>
      ...
      {analysis.diseaseInformation && analysis.diseaseInformation.length > 0 ? (
        <section
          className="disease-information"
          aria-labelledby="disease-information-title"
        >
          ...
          <div className="disease-information__list">
            {analysis.diseaseInformation.map((disease) => (
              ...
              >
                <header>
                  <div>
                    <span className="disease-information__query">
                      입력: {disease.query}
                    </span>
                    <h5>
                      {disease.matchedName}
                  ...
                  <Badge
                    tone={
                      disease.source === "official_api" ? "success" : "info"
                    }
                  >
                    {disease.sourceLabel}
                  </Badge>
                ...
                <p className="disease-information__overview">
                  {disease.overview}
                </p>
