...
test("issued sessions: sign in → withdraw → cancel recovery → restore → rewithdraw → expiry → same-Google rejoin", async ({ context, page, request }, info) => {
  ...
  try {
    ...
    evidence.push({ step: "explicit-recovery", cancelledLoginDidNotExtendDeadline: true, healthRestored: true, oldSessionsStillRejected: true, pushStillRemoved: true });
    ...
    evidence.push({ step: "three-calendar-month-expiry", beforeDeadlineRetained: true, expiredRecoveryStatus: 410, cronCalls, authRemoved: true, healthAndUnknownDescendantsRemoved: true, jobRemoved: true, otherAccountUnchanged: true });
    ...
    for (const uid of createdUsers) {
      ...
      await f.admin.recursiveDelete(f.admin.collection("careReadModels").doc(`google-${uid}`));
      await f.admin.collection("accountDeletions").doc(`google-${uid}`).delete();
      for (const name of ["pushSubscriptions", "medicationReminderSchedules", "medicationReminderSync", "pushDeliveries"]) {
        for (const row of (await f.admin.collection(name).where("recipientId", "==", `google-${uid}`).get()).docs) await row.ref.delete();
      }
    }
    await f.cleanup();
  }
